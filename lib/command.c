...
static int node_rbac_filter[VTY_NODE + 1];
static int rbac_filter_init = 0;

/* Check role permisson for command node. If command node is
  not in node_filter, it's assumed that checked successfully. */
static int check_node_permission(enum node_type ntype) {
    enum rbac_access role_access;
    int i;

    if (!rbac_filter_init) {
        for (i = 0; i <= VTY_NODE; ++i)
            node_rbac_filter[i] = RBAC_READ_ACCESS;

        //node_rbac_filter[ENABLE_NODE] = RBAC_WRITE_ACCESS;
        node_rbac_filter[CONFIG_NODE] = RBAC_WRITE_ACCESS;
        node_rbac_filter[INTERFACE_NODE] = RBAC_WRITE_ACCESS;
        node_rbac_filter[BGP_NODE] = RBAC_WRITE_ACCESS;
        node_rbac_filter[ZEBRA_NODE] = RBAC_WRITE_ACCESS;

        rbac_filter_init = 1;
    }

    role_access = rbac_get_access();
    if (ntype <= VTY_NODE)
        return role_access >= node_rbac_filter[ntype] ? 0 : 1;

    return 0;
}

struct cmd_rbac_access {
    const char *cmd_str;
    enum node_type ntype;
    enum rbac_access access;
};

/* There're few commands that need to be filtered, so it's feasible to use array as
     filter container. If the number of commands becomes larger, it's need to use
     hash to decrease the lookup time complexity */
static struct cmd_rbac_access cmd_rbac_filter[] = {
        {.cmd_str = "configure", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "write", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "start-shell", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "clear", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "debug", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "undebug", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS},
        {.cmd_str = "scp", .ntype = ENABLE_NODE, .access = RBAC_WRITE_ACCESS}
};

static int check_cmd_permission(enum node_type ntype, const char *cmd_str) {
    enum rbac_access role_access;
    int i, cnt, len;

    role_access = rbac_get_access();
    cnt = sizeof(cmd_rbac_filter) / sizeof(struct cmd_rbac_access);
    for (i = 0; i < cnt; ++i) {
        if (cmd_rbac_filter[i].ntype == ntype) {
            len = strlen(cmd_rbac_filter[i].cmd_str);
            if (!strncmp(cmd_rbac_filter[i].cmd_str, cmd_str, len))
                return role_access >= cmd_rbac_filter[i].access ? 0 : 1;
        }
    }
    return 0;
}
...
install_node(struct cmd_node *node,
             int (*func)(struct vty *)) {
    if (check_node_permission(node->node))
        return;
...
cmd_make_strvec(const char *string) {
    ...
    pipeflag=0;
    memset(ppipecmd,0, sizeof(ppipecmd));
...
install_element(enum node_type ntype, struct cmd_element *cmd) {
    ...
    if (check_node_permission(ntype) || check_cmd_permission(ntype, cmd->string))
        return;
...
boolean intf_range_match(const char *str) {
    int err, nm = 0;
    regmatch_t pmatch[nm];
    char *pattern = "^[0-9]+((-|,)[0-9]+){0,}$";
    regex_t reg;
    char errbuf[1024];
...
cmd_execute_command_real(vector vline,
    ...
    for (i = 0; i < vector_active (cmd_vector); i++)
    {
        if ((cmd_element = vector_slot (cmd_vector, i))) {
            if (cmd_is_complete(cmd_element, vline)) {
                //printf("wwwww cmd:%s, daemon:%d\n", cmd_element->string, cmd_element->daemon);
                //local command get higher priority than remote daemon.
                if (matched_count > 0) {
                    if ((cmd_element->daemon > 0) && (matched_element->daemon == 0)) {
                        continue;
                    } else if ((cmd_element->daemon == 0) && (matched_element->daemon > 0)) {
    ...
    if ((strcmp(tac_command, "exit") != 0) &&
        (strcmp(tac_command, "end") != 0) &&
        (strcmp(tac_command, "enable") != 0))
    {
        /* send command for authorization */
        ret = author_check_cmd(tac_command);
        if (ret == AUTHOR_FAIL)
        {
            vty_out(vty, "Cannot execute command. Command not allowed.\n");
            return CMD_ERR_NOTHING_TODO;
        }
        else if (ret == AUTHOR_CONN_ERR)
        {
...
cmd_init(int terminal) {
    ...
    host.lines = -1;
    host.motd = default_motd;
    host.motdfile = "/etc/sonic/banner";

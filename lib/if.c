...
void get_system_intfs(char *intfs[])
{
    FILE *fstream=NULL;
    char output[1024];

    memset(output,0,sizeof(output));
    if(NULL==(fstream=popen("docker exec -i swss cat /usr/share/sonic/hwsku/port_config.ini | grep Ether","r")))
    {
      zlog (NULL, LOG_INFO, "%s", safe_strerror(errno));
      return ;
    }
    ...
    while(NULL!=fgets(output, sizeof(output), fstream)) {
      ...
      intfs[++index] = strndup(tmp, strlen(tmp));
      //printf("index:%d, intf:%s\n", index, intfs[index]);
      memset(output,0,sizeof(output));
      if (feof(fstream))
        break;
    }

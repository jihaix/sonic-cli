...
int check_show_command(const char *p)
{
  const char *cp,*start;
  char *token;
  int strlen;
  ...
  *(token + strlen) = '\0';
  if(0==strcmp(token,"show")){
    ret=true;
  }
  XFREE(MTYPE_STRVEC,token);
  return ret;
}


int check_config_command(const char *p)
{
  const char *cp,*start;
  char *token;
  int strlen;
  ...
  *(token + strlen) = '\0';
  if((strcmp(token, "exit") != 0) &&
      (strcmp(token, "end") != 0) &&
      (strcmp(token, "enable") != 0) &&
      (strcmp(token, "configure") != 0) &&
      (strcmp(token, "show") != 0))
  {
     ret = true;
  }
...
int pickup_cmd(const char *string,char *cmd)
{
  const char *cp, *start;
  int strlen;
...
#define FIRSTNODE  1
#define SECONDNODE 2
static int vtysh_node_belong (enum node_type node)
{
  int ret=0;
  switch (node)
    {
...
void cmdlineset(enum node_type oldnode,enum node_type newnode,const char *cmdline)
{
  /*use end command over two node range,newnode must be enable-node*/
  if((oldnode==CONFIG_NODE)&&(vtysh_node_belong(newnode)==FIRSTNODE))
  {
    cmdcount=1;
    strcpy(vtypopline[0],cmdline);
  }
  else if((vtysh_node_belong(oldnode)==FIRSTNODE)&&(vtysh_node_belong(newnode)==SECONDNODE))
  {
    cmdcount=2;
    strcpy(vtypopline[1],cmdline);
  }
  else if((vtysh_node_belong(oldnode)==SECONDNODE)&&(vtysh_node_belong(newnode)==FIRSTNODE))
  {
    cmdcount=1;
    memset(vtypopline[1],'\0',sizeof(vtypopline[1]));
  }
  else/*newnode is lower or euql config-node*/
  {
    cmdcount=0;
    memset(vtypopline,'\0',sizeof(vtypopline));
  }
}
...
vtysh_execute_func (const char *line, int pager)
{
  ...
  enum node_type oldnode;

  char cmdstr[1024];
  char cmdstr_nopipe[1024];
  ...
  switch (ret)
  {
	 ...
	 * but is really ugly. */
        ...
        if (true==check_show_command(line)){
          if(pipeflag==1){
            pickup_cmd(line,cmdstr_nopipe);
            snprintf(cmdstr, 1024, "vtysh -c '%s' |%s", cmdstr_nopipe,ppipecmd);
          }
          else
          {
            snprintf(cmdstr, 1024, "vtysh -c '%s'", line);
          }
          ...
          for (i = 0; i < array_size(vtysh_client); i++)
          {
            ...
            {
              if (vty->node >= CONFIG_NODE) {
                cmd_stat = vtysh_client_execute(&vtysh_client[i], "configure terminal", fp);
                if(cmd_stat!=CMD_SUCCESS)
                {
                  ...
                  if(cmd_stat!=CMD_SUCCESS)
                  {
                    vty_out (vty, "%% quagga config terminal locked,try later.%s",VTY_NEWLINE);
                    break;
                  }
                }
                if((strcmp(line,"exit")!=0)&&(strcmp(line,"quit")!=0)&&(strcmp(line,"end")!=0))
                {
                  for(j=0;j<cmdcount;j++)
                  {
                    vtysh_client_execute(&vtysh_client[i], vtypopline[j], fp);
                  }
                }
              }
              ...
              if (vty->node >= CONFIG_NODE) {
                vtysh_client_execute(&vtysh_client[i], "end", fp);
              }
              if (cmd_stat != CMD_SUCCESS)
                break;
            }
          ...
          if (cmd->func){
            ...
            cmdlineset(oldnode,vty->node,line);
          }

          if (check_config_command(line))
          {
             vty_out (vty, "%%Command successfully, but not support to save in ConfigDB.%s",VTY_NEWLINE);
          }
        }
      }
  }
...
boolean ignore_cmd_match (const char *str)
{
    int err,nm = 0;
    regmatch_t pmatch[nm];
    char *pattern = "^(do|end|exit|quit)+";
    regex_t reg;
    char errbuf[1024];
...
int vtysh_execute_intf_range_wrapper(const char *line, int pager)
{
    if ( (vty->node == INTERFACE_NODE)
         && (vty->intf_range != NULL)
         && (strlen(line) != 0)
         && (!ignore_cmd_match(line)))
    {
        char *range = strdup(vty->intf_range);
        char *items[256];
        char *end_points[2];
        int num = 0, i = 0;
        int min = 0, max = 0, index = 0;
        int ret= 0;
        while((items[num] = strtok(range, ",")) != NULL)
        {
            num++;
            range = NULL;
            if (num > 256)
            {
                printf("ERR: interface range more than 256 items.\n");
                break;
            }
        }
        ...
        for (i=0;i < num;i++)
        {
            ...
            size_t sl;
            char lagName[24];
            //printf("num:%d, items:%s\n", num, items[i]);
            end_points[0] = strtok(items[i], "-");
            if ((end_points[1] = strtok(NULL, "-")) != NULL)
            {
                min = atoi(end_points[0]);
                max = atoi(end_points[1]);
                //printf("min:%d, max:%d\n", min, max);
                for (index = min;index <= max;index++)
                {
                    if (vty->intf_type == INTFACE_TYPE_PORT)
                    {
                        sl = strlen(system_intfs[index]);
                        ifp = if_get_by_name_len(system_intfs[index], sl);
                    }
                    else
                    {
                        sprintf(lagName, "PortChannel%d", index);
                        sl = strlen(lagName);
                        ifp = if_get_by_name_len(lagName, sl);
                    }
                    vty->index = ifp;
                    vty->node = INTERFACE_NODE;
                    //printf("cmd:%s, intf:%s\n", line, system_intfs[index]);
                    ret = vtysh_execute_func (line, pager);
                    if (ret != CMD_SUCCESS && ret != CMD_SUCCESS_DAEMON && ret != CMD_WARNING)
                    {
                        free(range);
                        return ret;
    ...
    {
        return vtysh_execute_func (line, pager);
    }
}
...
{
    return vtysh_execute_intf_range_wrapper(line, 0);
}

int
vtysh_execute (const char *line)
{
    return vtysh_execute_intf_range_wrapper(line, 1);
}
...
write_config_integrated(void)
{
  ...
  errno = 0;
  if ((quagga_vty_group = getgrnam("quagga")) == NULL)
    {
      fprintf (stdout, "%% Can't get group quagga: %s (%d)\n",
...
int execute_linux_cmd_pipe(char* cmd)
{
  FILE *fstream=NULL;
  char output[1024];
  char cmdline[1024];
  ...
  if (cmd == NULL)
  {
    vty_out(vty,"execute command failed: cmd is NULL");
    return -1;
  }

  if(pipeflag==1){
    sprintf(cmdline,"%s |%s",cmd,ppipecmd);
  }
  else{
  ...
  if(-1 == rc)
  {
    vty_out(vty,"close command file failed");
    exit(1);
  }
  else
  {
    if(WEXITSTATUS(rc)!=0)
    {
      syslog(LOG_ERR,"execut command  %s failed: %d ",cmdline,rc);
    }
  }
  return 0;
}
